...
class C45decisionTree:
    ...
    def npredict(self, X__: ndarray, y_: ndarray) -> float:
      """计算测试正确率"""
      测试正确样本数 = sum(self.predict(X__)==y_)
      print('预测结果')
      print(self.predict(X__))
      总样本数 = len(y_)
      print('预测准确率')
      print(测试正确样本数/总样本数)
...
if __name__=='__main__':
    ...
    model = C45decisionTree(
            minSamplesLeaf=2,  # 超参数：叶结点的最少样本数量
            maxDepth=3,        # 超参数：最大树深
            maxPruity=1.,      # 超参数：叶结点的最大纯度
            # maxFeatures=4,     # 超参数：最大特征数
            α=0.3,              # 超参数：代价复杂度剪枝的惩罚参数
            )
...
print(f'预测样本集的召回率：{recall:.4f}')
    ...
    #预测新数据的代码
path=r'/Users/yaoji/reins/论文相关/毕业设计/iris2.csv'
pdata=pd.read_csv(path)
pdata=np.array(pdata)
nX = pdata[:, :-1]     
ny = pdata[:, -1]

model.npredict(nX, ny)
