...
class ID3decisionTree:
    ...
    def chooseBestFeature(self, X__: ndarray, y_: ndarray,
        ...
        info_gain_dict = {}  # 初始化一个空字典来存储每个特征的信息增益
            ...
            if m in self.indexContinuousFeatures_:
                ...
                for t in t_:
                    ...
                    if  informationGain > informationGains_[m]:
                        ...
                        print(f'Feature {m} information gain: {informationGains_[m]}')
            else:
                # 若第m特征为离散值特征
                ...
                for value in values_:
                    ...
                    if len(ySubset_) == 0:
                       # 若按第m特征取值为value划分得到的子样本集为空，则跳过此次循环
                       continue
                    p = len(ySubset_)/N                # 第m特征取值为value的样本所占比例
                    newEntropy += p*entropy(ySubset_)  # 多个子样本集的熵值
                informationGains_[m] = round(baseEntropy - newEntropy, 2)  # 按第m特征划分所获得的信息增益
    ...
    def precision(self, X__: ndarray, y_: ndarray) -> float:
        """计算测试精确率"""
        y_pred = self.predict(X__)
        精确率 = precision_score(y_, y_pred, average='macro')
        return 精确率

    def recall(self, X__: ndarray, y_: ndarray) -> float:
        """计算测试召回率"""
        y_pred = self.predict(X__)
        召回率 = recall_score(y_, y_pred, average='macro')
        return 召回率
...
if __name__=='__main__':
    ...
    indexContinuousFeatures_ = (3,1,2,4)  # 连续值特征的序号

    feature_names = ['sepal length', 'sepal width', 'petal length', 'petal width']

    # 实例化ID3决策树模型
    model = ID3decisionTree(
            minSamplesLeaf=4,  # 超参数：叶结点的最少样本数量
            maxDepth=7,        # 超参数：最大树深
            maxPruity=1.,      # 超参数：叶结点的最大纯度
            # maxFeatures=4,     # 超参数：最大特征数
            α=2.5,              # 超参数：代价复杂度剪枝的惩罚参数
            )
    model.fit(Xtrain__, ytrain_,    # 训练样本特征向量矩阵、训练样本标签
              indexContinuousFeatures_=indexContinuousFeatures_,  # 连续值特征的序号
              )      # 训练
    model.pruning()  # 剪枝
    print(f'预测样本集的预测正确率：{model.accuracy(Xtest, ytest):.4f}\n')
    print(f'预测样本集的精确率：{model.precision(Xtest, ytest):.4f}\n')
    print(f'预测样本集的召回率：{model.recall(Xtest, ytest):.4f}\n')
# 可视化未剪枝的决策树
dot = model.export_graphviz(model.tree)
dot.render('/Users/yaoji/reins/论文相关/ID3decision_tree_before_pruning', format='pdf', cleanup=True)
