...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 23 00:59:57 2023
...
class C45decisionTree:
    """C4.5决策树分类"""
    def __init__(self,
            minSamplesLeaf: int = 5,            # 超参数：叶结点的最少样本数量
            maxDepth: int = 2,                  # 超参数：最大树深
            maxPruity: float = 8,              # 超参数：叶结点的最大纯度
    ...
    def export_graphviz(self, node: dict, label: str = '', parent: str = None) -> Digraph:
        ...
        if 'child nodes' not in node:
            # 如果是叶子节点
            label = f'Class: {node.get("class", "unknown")}\nSamples: {node["number of samples"]}\nEntropy: {node["entropy"]:.2f}'
            dot.node(name=str(id(node)), label=label, shape='box')
        else:
            # 如果是内部节点
            index_of_splitting_feature = node.get("index of splitting feature", "unknown")
            gain_ratio = node.get("gain ratio", "unknown")
            gain_ratio_str = f"{gain_ratio:.2f}" if isinstance(gain_ratio, float) else "unknown"
            label = f'Feature: {index_of_splitting_feature}\nGain Ratio: {gain_ratio_str}\nSamples: {node["number of samples"]}\nEntropy: {node["entropy"]:.2f}'
            dot.node(name=str(id(node)), label=label, shape='ellipse')
            for key in node['child nodes']:
                child_node = node['child nodes'][key]
                dot.edge(str(id(node)), str(id(child_node)), label=f'{node["index of splitting feature"]} = {key}')
    ...
    def evaluate(self, X_test: ndarray, y_test: ndarray) -> Tuple[float, float]:
        """计算测试集上的精确度和召回率"""
        y_pred = self.predict(X_test)  # 对测试集进行预测
...
if __name__=='__main__':
    ...
    print('使用iris数据集进行试验\n')
    DataSet = pd.read_csv(r'/Users/yaoji/reins/论文相关/毕业设计/iris2.csv')
    DataSet = np.array(DataSet)

    X = DataSet[:, :-1]     # 样本特征向量矩阵
    y = DataSet[:, -1]       # 样本标签
    Xtrain__, Xtest, ytrain_, ytest = train_test_split(X, y, test_size = 0.3, random_state = 272)

    indexContinuousFeatures_ = (1,2,3,4)  # 连续值特征的序号

    # 实例化C4.5决策树模型
    model = C45decisionTree(
            minSamplesLeaf=6,  # 超参数：叶结点的最少样本数量
            maxDepth=2,        # 超参数：最大树深
    ...
    model.pruning()  # 剪枝
    print(f'预测样本集的预测正确率：{model.accuracy(Xtest, ytest):.4f}\n')
    precision, recall = model.evaluate(Xtest, ytest)
print(f'预测样本集的精确度：{precision:.4f}')
print(f'预测样本集的召回率：{recall:.4f}')


# 可视化未剪枝的决策树
dot = model.export_graphviz(model.tree)
dot.render('/Users/yaoji/reins/论文相关/decision_tree_before_pruning', format='pdf', cleanup=True)
...
# 可视化剪枝后的决策树
dot = model.export_graphviz(model.tree)
dot.render('/Users/yaoji/reins/论文相关/decision_tree_after_pruning', format='pdf', cleanup=True)
